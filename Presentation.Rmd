...
4. Team valuations from individual salaries?
...
## Including all predictors lead to issues (hello, collinearity). So, we got rid of some (e.g. Offensive Rebounds) and kept others (e.g. Total Rebounds).
...
## Are we better suited than by a non-linear model?
### Ploting prominent variables against SALARY
...
```{r, message=FALSE, echo=FALSE}
eda_plot <- ggplot() +
  geom_point(data = players_with_stats_salary, aes(x = `3P`, y = POINTS), color="black") +
  geom_point(data = players_with_stats_salary, aes(x = FT, y = POINTS), color="blue") +
  geom_point(data = players_with_stats_salary, aes(x = FG, y = POINTS), color="green") +
  geom_point(data = players_with_stats_salary, aes(x = `2P`, y = POINTS), color="pink") +
  xlab("Performance Statistics Points") + ylab("Points per Game") +
  ggtitle("Points Relative to other Performance Stats")
...
# rmse_tab <- tibble(rmse = rmse(ppg_model, test_set_points)) %>%
#   kable("html") %>%
#   kable_styling(bootstrap_options = "striped", full_width = F, position = "left")
...
### Wow, that looks nearly perfect! We also get an RMSE of just 0.07464396.
#### Also, let's check to make sure that "Points" isn't simply just the sum of our predictors. It's not!

<br>

```{r, message=FALSE, echo=FALSE}
test_set_points <- test_set_points %>%
  mutate(summer = FT+`3P`+FG+`2P`)

mini_table <- tibble(original_points = (test_set_points$POINTS)[1:10])
mini_table <- mini_table %>%
  mutate(pred_points = round((test_set_points$pred)[1:10], 2),
...
# Question 3: Salary based on social media stats or vice versa?
### These results are not great... check out some quick graphs using a linear model
